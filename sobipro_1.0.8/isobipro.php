...
class Isobipro extends SPEntryCtrl
{
	 ...
	 * @return              typeArrs
	 */
	public function gettypes($fieldid)
	{
		if (WEBSITE_FIELD)
		{
	 ...
	 * @return             typeArrs
	 */
	public function getDealTypes($fieldid)
	{
		if (ADDRESS1_FIELD)
		{
			$typeadd1Fields = explode(',', ADDRESS1_FIELD);
	...
	public function sectionCategories()
	{
		...
		foreach ($catid as $cid)
		{
			...
			elseif (strtotime($category->get('validUntil')) != 0 && strtotime($category->get('validUntil')) < time())
			{
				$cat['state'] = 'expired';
			}
		...
		elseif ($sectionID != '' && !($catID) && $latitude == 0 && $longitude == 0)
		{
			$implode = implode(',', $catid);
			$query   = "SELECT so.id
					FROM #__sobipro_object as so
		...
		foreach ($entryIDs as $enid)
		{
			...
			{
				$en['url'] = Sobi::Url(array('title' => $entry->get('name'), 'pid'   => $entry->get('primary'), 'sid' => $entry->get('id')));
			}
			else
			{
				$en['url'] = Sobi::Url(array('title' => $entry->get('name'), 'pid'   => SPRequest::sid(), 'sid' => $entry->get('id')));
			}
			...
			elseif (strtotime($entry->get('validUntil')) != 0 && strtotime($entry->get('validUntil')) < time())
			{
				$en['state'] = 'expired';
			}
		...
		for ($inc = $eLimStart; $inc < $looplimit; $inc++)
		{
			...
			if ($reviews)
			{
				...
				{
					unset($reviews['summary_rating']);
					unset($reviews['navigation']);
			...
			foreach ($fields as $fkey => $fval)
			{
				...
				if (($caption == 'Company Image' || $caption == 'Image') && (isset($value['img']['_attributes']['src']) && !empty($value['img']['_attributes']['src'])))
				{
				...
				else
				{
					...
					elseif ($unit == '[cfg:payments.currency]')
					{
					...
					elseif ($type == 'chbxgroup' || $type == 'multiselect')
					{
						$days = $value['ul']['_data'];
	 ...
	 *
	 * @param   integer  $tsid    tsid
	 * @param   boolean  $bypass  bypass
	 *
	 * @return array/boolean  true on success and false on failure and Jsonarray
	 */
	public function getsearchField($tsid = 0, $bypass = false)
	{
		...
		foreach ($fields as $key => $value)
		{
			...
			elseif ($value['fieldType'] == 'calendar')
			{
	...
	public function getsearch()
	{
		...
		elseif ($inputForm['field_deal_start'] == 'dd-mm-yy')
		{
			$start_from_selector = date("d-m-Y H:i", strtotime($this->_request['field_deal_start']['from']));
			$start_to_selector   = date("d-m-Y H:i", strtotime($this->_request['field_deal_start']['to']));
		}
		elseif ($inputForm['field_deal_start'] == 'mm/dd/yy')
		{
		...
		elseif ($inputForm['field_deal_start'] == 'yy-mm-dd')
		{
		...
		elseif ($inputForm['field_deal_start'] == 'd.m.yy')
		{
			$start_from_selector = date("d.m.Y H:i", strtotime($this->_request['field_deal_start']['from']));
			$start_to_selector   = date("d.m.Y H:i", strtotime($this->_request['field_deal_start']['to']));
		}
		...
		elseif ($inputForm['field_deal_end'] == 'dd-mm-yy')
		{
			$end_from_selector = date("d-m-Y H:i", strtotime($this->_request['field_deal_end']['from']));
			$end_to_selector   = date("d-m-Y H:i", strtotime($this->_request['field_deal_end']['to']));
		}
		elseif ($inputForm['field_deal_end'] == 'mm/dd/yy')
		{
		...
		elseif ($inputForm['field_deal_end'] == 'yy-mm-dd')
		{
		...
		elseif ($inputForm['field_deal_end'] == 'd.m.yy')
		{
			$end_from_selector = date("d.m.Y H:i", strtotime($this->_request['field_deal_end']['from']));
			$end_to_selector   = date("d.m.Y H:i", strtotime($this->_request['field_deal_end']['to']));
		}
	 ...
	 */
	public function getResults($entryIDs)
	{
		...
		for ($inc = $eLimStart; $inc < $looplimit; $inc++)
		{
			...
			foreach ($fields as $fkey => $fval)
			{
				...
				elseif (isset($unit) && $unit != '[cfg:payments.currency]')
				{
	 ...
	 * @return          fields
	 */
	public function loadFields($sid)
	{
		$fields = null;
	 ...
	 * @return  some value
	 */
	public function sortByPrio($obj, $to)
	{
	 ...
	 * @return         results
	 */
	public function searchWords($all, $sid)
	{
		// @TODO categories
		$matches = array();
		...
		if (count($matches) && isset($matches[0]))
		{
			...
			if (count($wordResults))
			{
				foreach ($wordResults as $wordResult)
				{
					...
					elseif ($all && is_array($wordResult))
					{
						$results = array_intersect($results, $wordResult);
					}
					elseif (is_array($wordResult))
					{
	 ...
	 * @return  void
	 */
	public function searchPhrase($sid)
	{
		/* @TODO categories */
	 ...
	 * @return   results
	 */
	public function travelFields($word,  $sid, $regex = false)
	{
		$results = array();
	 ...
	 * @return  some value
	 */
	public function addentryField($tsid = 0, $bypass = false)
	{
		$sectionID = IJReq::getTaskData('sectionID', $tsid);
		$form      = IJReq::getTaskData('form');
		SPFactory::registry()->set('current_section', json_decode(json_encode($sectionID), false));
