...
class user{
    ...
    function like(){
    	...
    	if($this->jsonarray = $this->jomHelper->Like('profile',$userID)){
    		$this->jsonarray['code']=200;
    		return $this->jsonarray;
    	}else{
			...
			if($field->type == 'group'){
				...
				if(isset($field->options) && !empty($field->options)){
					...
					foreach($option as $val){
						$this->jsonarray['fields']['group'][$inc]['field'][$incj]['options'][$i]['value']=JText::_($val);
						$i++;
					}
				}
				$incj++;
			...
			switch($value->cmd_type){
				...
				case "notif_inbox_create_message":
				 ...
				 case "notif_groups_wall_create":
				 	$params = new CParameter( $value->params );
					$usr = $this->jomHelper->getUserDetail($value->actor);
					$this->jsonarray['notifications']['global'][$ind]['notif_title']		= str_replace('{actor}',$usr->name,str_replace('{group}',$params->get('group'),$value->content));
					$this->jsonarray['notifications']['global'][$ind]['type']			= 'profile';
					$this->jsonarray['notifications']['global'][$ind]['user_id']			= $usr->id;
					$this->jsonarray['notifications']['global'][$ind]['user_name']		= $usr->name;
					$this->jsonarray['notifications']['global'][$ind]['user_avatar']		= $usr->avatar;
				 ...
				 case "notif_groups_create_news":
						$params = new CParameter( $value->params );
						$usr = $this->jomHelper->getUserDetail($value->actor);
						$this->jsonarray['notifications']['global'][$ind]['notif_title']		= str_replace('{actor}',$usr->name,str_replace('{group}',$params->get('group'),str_replace('{announcement}',$params->get('announcement'),$value->content)));
						$this->jsonarray['notifications']['global'][$ind]['type']			= 'profile';
						$this->jsonarray['notifications']['global'][$ind]['user_id']			= $usr->id;
						$this->jsonarray['notifications']['global'][$ind]['user_name']		= $usr->name;
						$this->jsonarray['notifications']['global'][$ind]['user_avatar']		= $usr->avatar;
						...
						$ind++;
						break;
				case "notif_groups_create_discussion":
						$params = new CParameter( $value->params );
						$usr = $this->jomHelper->getUserDetail($value->actor);
						$this->jsonarray['notifications']['global'][$ind]['notif_title']		= str_replace('{actor}',$usr->name,str_replace('{group}',$params->get('group'),str_replace('{discussion}',$params->get('discussion'),$value->content)));
						$this->jsonarray['notifications']['global'][$ind]['type']			= 'profile';
						$this->jsonarray['notifications']['global'][$ind]['user_id']			= $usr->id;
						$this->jsonarray['notifications']['global'][$ind]['user_name']		= $usr->name;
						$this->jsonarray['notifications']['global'][$ind]['user_avatar']		= $usr->avatar;
				 ...
				 default:
					$params = new CParameter( $value->params );
					//print_r($params);
					$srch = array('{actor}','{photo}','{album}','{video}');
					$rplc = array($usr->name,$params->get('photo'),$params->get('album'),$params->get('video'));
					$usr = $this->jomHelper->getUserDetail($value->actor);
					$this->jsonarray['notifications']['global'][$ind]['notif_title']		= str_replace($srch,$rplc,$value->content);
					$this->jsonarray['notifications']['global'][$ind]['type']			= 'profile';
					$this->jsonarray['notifications']['global'][$ind]['user_id']			= $usr->id;
