...
class jomHelper
{
	 ...
	 * @param   boolean  $password  password
	 ...
	 * @param   integer  $deviceRegistrationId  device registration id
	 * @param   string   $msgType               message type
	 * @param   string   $messageText           message text
	 * @param   string   $totMsg                total message
	 * @param   [type]   $whentype              [description]
	 *
	 * @return  void
	 */
	 ...
	 * @param   integer  $friendID  freind id
	 *
	 * @return  boolean             access_limit
	 */
	 ...
	 *
	 * @param   integer  $userID     id of user
	 * @param   [type]   $frontUser  front user
	 *
	 * @return  boolean              user
	 */
	...
	public function getTitleTag($html_data)
	{
		...
		if ($html_data->params)
		{
			$action = $html_data->params->get('action');
		}
		...
		switch ($html_data->app)
		{
			...
			case 'photos':
				if ($html_data->params->get('style') == COMMUNITY_STREAM_STYLE || strpos($html_data->title, '{multiple}'))
				{
					$count = $html_data->params->get('count', 1);

					if (CStringHelper::isPlural($count))
					{
						$titletag = $username . JText::sprintf('COM_COMMUNITY_ACTIVITY_PHOTO_UPLOAD_TITLE_MANY', $count, '', $html_data->album->name);
					}
					else
					{
						$titletag = $username . JText::sprintf('COM_COMMUNITY_ACTIVITY_PHOTO_UPLOAD_TITLE', '', $html_data->album->name);
					}
				}
				break;
			...
			case 'events':
				...
				$actors = $html_data->params->get('actors');
			...
			case 'events.avatar.upload':
				$event = JTable::getInstance('Event', 'CTable');
				$event->load($html_data->eventid);
				$titletag = $username . " " . JText::sprintf('COM_COMMUNITY_CHANGE_EVENT_S_AVATAR', $event->getLink(), $event->title);
				break;
			...
			case 'groups.avatar.upload':
				...
				$titletag = JText::sprintf('COM_COMMUNITY_GROUPS_NEW_GROUP_AVATAR', CRoute::_($group->getLink()), $group->name);
				break;
			...
			case 'albums':
				...
				$wall->load($html_data->params->get('wallid'));
			...
			case 'cover.upload':
				...
				//
				$sql = "SELECT params FROM #__community_activities WHERE `id`=" . $htmlId;
	...
	public function getAlbumContent($html_data)
	{
		$db      = JFactory::getDBO();
		$param   = new CParameter($html_data->params);
		$photoid = $html_data->params->get('photoid', false);
		$count   = $html_data->params->get('count', 1);
