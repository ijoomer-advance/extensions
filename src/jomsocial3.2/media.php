...
class media
{
	...
	function uploadPhoto()
	{
		...
		$imageCount = IJReq::getTaskData('imageCount', 0, 'int');
		if ($imageCount)
		{
			$photos = array();
			for ($i = 1; $i <= $imageCount; $i++)
			{
				$images     = JRequest::getVar('image' . $i, '', 'FILES');
				$photos[$i] = $images;
			}
			...
			$act->comment_id   = $album->id;

			// Allow like on the album
			$act->like_type = 'albums';
			$act->like_id   = $album->id;
		...
		$params->set('photo_url', $photoTable->getThumbURI());
		$params->set('count', 1);
		$params->set('batchcount', 1);
		$params->set('photosId', $photoTable->id);
	...
	/**
	 * _getRequestUserAlbum function
	 *
	 * @param   [type]  $value  value
	 *
	 * @return  boolean          album
	 */
	function _getRequestUserAlbum($value)
	{
	 ...
	 * @param   [type]  $limitSetting  limit setting
	 *
	 * @return array jsonarray
	 */
	 ...
	 * @param   integer  $deletedvideoid  id of deleted video
	 *
	 * @return  boolean                  returns value
	 */
	 ...
	 *
	 * @param   integer  $id           id
	 * @param   boolean  $returnThumb  returns thumb
	 *
	 * @return  boolean                 returns value
	 */
