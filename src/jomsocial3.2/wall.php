...
class wall
{
	...
	function wall()
	{
		...
		switch ($type)
		{
			...
			case 'group':
				$options = array(
					...
					'date'            => '',
					'maxList'         => MAXIMUM_WALL + 1,
					'app'             => array('groups.wall', 'groups.attend', 'events.wall', 'videos', 'groups.discussion', 'groups.discussion.reply', 'groups.bulletin', 'photos', 'events', 'cover.upload', 'groups.avatar.upload'),
					'cid'             => '',
					'groupid'         => $uniqueID,
					'eventid'         => '',
					'exclusions'      => '',
		...
		foreach ($htmldata as $key => $html)
		{
			...
			else
			{
				...
				$usr                                                       = $this->jomHelper->getRequiredUserDetail($html->actor);
				$this->jsonarray['update'][$inc]['user_detail']['user_id'] = $usr->id;

				if (!empty($html->target) && $html->target != $html->actor)
				{
					$targetUser                                                  = $this->jomHelper->getRequiredUserDetail($html->target);
				...
				switch ($html->app)
				{
					case 'friends':
						...
						$usrtar                                                          = $this->jomHelper->getRequiredUserDetail($html->target);
					...
					case 'photos':
						...
						$photoid = $html->params->get('photoid', false);
					...
					case 'groups.bulletin':
						...
						if ($bulletin->id)
						{
							...
							$usr                                                               = $this->jomHelper->getRequiredUserDetail($bulletin->created_by);
					...
					case 'groups.discussion':
						...
						if ($discussion->id)
						{
							...
							$usr                                                             = $this->jomHelper->getRequiredUserDetail($discussion->creator);
							$this->jsonarray['update'][$inc]['content_data']['user_id']      = $usr->id;
							$this->jsonarray['update'][$inc]['content_data']['user_name']    = $usr->name;
							$this->jsonarray['update'][$inc]['content_data']['user_avatar']  = $usr->avatar;
							$this->jsonarray['update'][$inc]['content_data']['user_profile'] = $usr->profile;
					...
					case 'profile.avatar.upload':
						...
						$attachment = $html->params->get('attachment');

						$this->jsonarray['update'][$inc]['image_data']['url'] = JURI::base() . $attachment;
						$actor                                                = CFactory::getUser($html->actor);
						$srch                                                 = array('{actor}', "&#9658;", "&quot;");
						$rplc                                                 = array($actor->getDisplayName(), "►", "\"");
						$this->jsonarray['update'][$inc]['titletag']          = str_replace($srch, $rplc, strip_tags($titletag));
					...
					case 'groups.avatar.upload':
						...
						if ($group->avatar)
						{
							$this->jsonarray['update'][$inc]['image_data']['url'] = JURI::base() . $group->avatar;
						}
						else
						{
							$this->jsonarray['update'][$inc]['image_data']['url'] = '';
						}
						...
						$rplc                                             = array($actor->getDisplayName(), "►", "\"");
						$this->jsonarray['update'][$inc]['titletag']      = str_replace($srch, $rplc, strip_tags($titletag));
						$this->jsonarray['update'][$inc]['type']          = 'groups.avatar.upload';
						$this->jsonarray['update'][$inc]['deleteAllowed'] = intval((($html->actor == $this->my->id) || $isCommunityAdmin) && ($this->my->id != 0));//intval($this->my->authorise('community.delete','activities.'.$html->id));
					...
					case 'events.avatar.upload':
						...
						if ($event->avatar)
						{
							$this->jsonarray['update'][$inc]['image_data']['url'] = JURI::base() . $event->avatar;
						}
						else
						{
							$this->jsonarray['update'][$inc]['image_data']['url'] = '';
						...
						$rplc                                             = array($actor->getDisplayName(), "►", "\"");
						$this->jsonarray['update'][$inc]['titletag']      = str_replace($srch, $rplc, strip_tags($titletag));
						$this->jsonarray['update'][$inc]['type']          = 'events.avatar.upload';
						$this->jsonarray['update'][$inc]['deleteAllowed'] = intval((($html->actor == $this->my->id) || $isCommunityAdmin) && ($this->my->id != 0));//intval($this->my->authorise('community.delete','activities.'.$html->id));
					...
					case 'cover.upload':
						...
						$domain                                               = JURI::base();
						$url                                                  = str_replace($domain, '', $attachment);
						$this->jsonarray['update'][$inc]['image_data']['url'] = JURI::base() . $url;
						$actor                                                = CFactory::getUser($html->actor);
						$srch                                                 = array('{actor}', "&#9658;", "&quot;");
						$rplc                                                 = array($actor->getDisplayName(), "►", "\"");
	...
	private function _getData($options)
	{
		...
		$blockLists    = $model->getBlockedList($this->my->id);
		$blockedUserId = array();
	...
	function add()
	{
		...
		if (!empty($message))
		{
			...
			if ($this->IJUserID != $recipient->id)
			{
				...
				$usr          = $this->jomHelper->getRequiredUserDetail($this->my->id);
		...
		if ($this->my->authorise('community.add', 'activities.comment.' . $act->actor, $obj))
		{
			...
			if ($act->groupid == 0 && $act->eventid == 0)
			{
				...
				if ($this->my->id != $act->actor)
				{
					...
					// get user push notification params and user device token and device type
					$usr     = $this->jomHelper->getRequiredUserDetail($this->IJUserID);
					$match   = array('{actor}', '{stream}');
					$replace = array($usr->name, JText::_('COM_COMMUNITY_SINGULAR_STREAM'));
					$message = str_replace($match, $replace, JText::sprintf('COM_COMMUNITY_ACITIVY_WALL_EMAIL_SUBJECT'));
		...
		if ($like_fields->like_id)
		{
			...
			foreach ($rows as $row)
			{
				...
				if ($row->like)
				{
					...
					{
						$usr                                            = $this->jomHelper->getRequiredUserDetail($userlike);
	...
	function getComments()
	{
		...
		if ($comment_act->comment_id > 0)
		{
			...
			{
				$usr = $this->jomHelper->getRequiredUserDetail($comment->post_by);
				CFactory::load('libraries', 'comment');
